...
public class ListenForSoundQueuesW : Action
{

    public AIStats stats;
    public SharedVector3 target2;
    ...
    public override TaskStatus OnUpdate()
    {
        ...
        if (heardObjects.Length > 0)
        {
            ...
            if (heardObjects[0].transform.position != target2.Value)
            {
                Debug.Log($"{heardObjects[0].name} at {heardObjects[0].transform.position}");
                target2.Value = heardObjects[0].transform.position;
